...
export default function RadialOrbitalTimeline({
  ...
  return (
    ...
    >
      {/* Title */}
      <div style={{ position: 'absolute', top: 40, textAlign: 'center', zIndex: 50 }}>
        <h1 style={{ fontSize: 36, fontWeight: 800, letterSpacing: '-0.02em' }}>
          <span className="gradient-text">DevProof</span>
        </h1>
        <p style={{ color: 'rgba(255,255,255,.5)', fontSize: 14, marginTop: 6 }}>Developer Trust Platform — Click a feature to explore</p>
      </div>
                      ...
                      {item.route && onNavigate && (
                        <div className="mt-4 pt-3 border-t border-white/10">
                          <Button
                            variant="outline"
                            size="sm"
                            className="w-full flex items-center justify-center h-8 text-xs rounded-md border-white/30 bg-white/5 hover:bg-white/15 text-white hover:text-white transition-all"
                            onClick={(e) => {
                              e.stopPropagation();
